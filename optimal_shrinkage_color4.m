function [Y_os,eta,r_p,eta_a] = optimal_shrinkage_color4(Y,loss,k,method,cm,c)
...
for j = 1:r_p
    ...
    if method == "cut"
        ...
        m1 = (1/(p-k) *sum(1./(lab((k+1):end)-ov(j))));
        dm1 = (1/(p-k) *sum(1./(lab((k+1):end)-ov(j)).^2));
    ...
    a1 = abs(m1/(d^2*dTau)); a2 = abs(m2/(d^2*dTau));
