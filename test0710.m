...
%x = [4,2.1]; gamma = 1; name1 = 'Unif[1,10]';name2 = 'Mix2.2';
x = [3,1.9];gamma = 0.5;name1 = 'Unif[1,10]';name2 = 'Mix2.2';
%x = [4,1.5];gamma = 1;name1 = 'Marcenko-Pastur';name2 = 'Marcenko-Pastur';
%x = [3,1.3];gamma = 0.5;name1 = 'Marcenko-Pastur';name2 = 'Marcenko-Pastur';
r = length(x);
cm = 10; % student t 10 's moment
c = 4;
kk = 2; % k for imputation
smth = '';
%rng(1);
...
for i = 1:length(ps)
    ...
    for t  = 1:T
        ...
        [~,eta_f,~,~] = optimal_shrinkage_color4(Y,sh,kk,method,cm,c);
        hX_f = U*diag(eta_f)*V(:,1:p)';
        err_f = norm(X-hX_f,err_method);
        errF = [errF; err_f];

        [~,eta_f2,~,~] = optimal_shrinkage_color4(Y,sh,kk,'cut',cm,c);
        hX_f2 = U*diag(eta_f2)*V(:,1:p)';
        err_f2 = norm(X-hX_f2,err_method);
        errF2 = [errF2; err_f2];

        %[~,eta_o] = optimal_shrinkage_color2(Y,'op',r_p,method);
        [~,eta_o,r_p,eta_a] = optimal_shrinkage_color4(Y,'op',kk,method,cm,c);
    ...
    mD1 = [mD1, errD(:,1)];mD2 = [mD2, errD(:,2)];
    mD1_2 = [mD1_2, errD2(:,1)];mD2_2 = [mD2_2, errD2(:,2)];
    mDSN1 = [mDSN1, errDSN(:,1)];mDSN2 = [mDSN2, errDSN(:,2)];
    mDW1 = [mDW1, errDW(:,1)];mDW2 = [mDW2, errDW(:,2)];
    mDW1_2 = [mDW1_2, errDW2(:,1)];mDW2_2 = [mDW2_2, errDW2(:,2)];
    mD01 = [mD01, errD0(:,1)];mD02 = [mD02, errD0(:,2)];
    mA1 = [mA1, errA(:,1)];mA2 = [mA2, errA(:,2)];
    mA1_2 = [mA1_2, errA2(:,1)];mA2_2 = [mA2_2, errA2(:,2)];
...
errorbar(ps,median(abs(mD1_2./x(1)*100)),median(abs(mD1_2./x(1)*100))-quantile(abs(mD1_2./x(1)*100),.25),quantile(abs(mD1_2./x(1)*100),.75)-median(abs(mD1_2./x(1)*100)),'b','Linewidth', 1);
errorbar(ps,median(abs(mD2_2./x(2)*100)),median(abs(mD2_2./x(2)*100))-quantile(abs(mD2_2./x(2)*100),.25),quantile(abs(mD2_2./x(2)*100),.75)-median(abs(mD2_2./x(2)*100)),'--b','Linewidth', 1);
