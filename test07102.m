...
for i = 1:length(ps)
    ...
    for t  = 1:T
        ...
        [~,eta_f,~,~] = optimal_shrinkage_color3(Y,sh,kk,method,cm,c);
        hX_f = U*diag(eta_f)*V(:,1:p)';
        err_f = norm(X-hX_f,err_method);
        errF = [errF; err_f];

        [~,eta_f2,~,~] = optimal_shrinkage_color3(Y,sh,kk,'cut',cm,c);
        hX_f2 = U*diag(eta_f2)*V(:,1:p)';
        err_f2 = norm(X-hX_f2,err_method);
        errF2 = [errF2; err_f2];

        %[~,eta_o] = optimal_shrinkage_color2(Y,'op',r_p,method);
        [~,eta_o,r_p,eta_a] = optimal_shrinkage_color3(Y,'op',kk,method,cm,c);
        [~,eta_o2,~,eta_a2] = optimal_shrinkage_color3(Y,'op',kk,'cut',cm,c);
        a = [];
        for k = 1:length(x)
        ...
        whX_fro2 = optimal_shrinkage_color3(wY,sh,kk,'cut',cm,c);hX_fro2 = (hA.^(1/2))*whX_fro2*(hB.^(1/2));
